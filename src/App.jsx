...
// Grab a reference to the vote module
...
const App = () => {
  ...
  // Need to check if the user has already voted on the proposal.
  useEffect(() => {
    if (!hasClaimedNFT) {
      return;
    }

    // If we haven't finished retreiving the proposals from the useEffect above then we can't check if the user 
    // voted yet.
    if (!proposals.length) {
      return;
    }
  ...
  if (hasClaimedNFT) {
    return (
      ...
      <h1>EffectiveAltruistsDAO Member Page</h1>
